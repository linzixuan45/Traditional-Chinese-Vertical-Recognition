...
class BaiduOcr():
    ...
    def img_2_word(self, img_path, st_size=M4):
        """
        将图片识别为文字
        :param img_path:
        :return: 返回图片上所有的文字
    ...
    def img_2_word_acc(self, img_path, st_size=M10):
        url_acc = "https://aip.baidubce.com/rest/2.0/ocr/v1/accurate_basic?access_token=" + self.ACCESS_TOKEN
        payload = {
            "image": self.img_2_base64(img_path, st_size)
        }
        headers = {
    ...
    def inference(self, img_path, img_save_path, acc_loc=False, dpi=300):
        ...
        if acc_loc:
            json_dict_1 = self.img_2_word_acc_loc(img_path)
            json_dict_2 = self.img_2_word(img_path)
        else:
        ...
        if acc_loc:
            if list(json_dict_1.keys())[1] in ['error_msg']:
                return json_dict_1['error_msg'], img_path, json_dict_1
            else:
                img_save_path = plt_img_info(img_path, json_dict_1, img_save_path, dpi=dpi)
                poem_sort, loc_combine = acc_loc_poem(json_dict_1)
                poem_sample = [value['words'] for value in json_dict_2['words_result']]
                acc_poem = get_acc_poem(poem_sort, loc_combine, poem_sample)
                return acc_poem, img_save_path, json_dict_1
            ...
            if list(json_dict_1.keys())[1] in ['error_msg']:
                ...
                img_save_path = plt_img_info(img_path, json_dict_1, img_save_path, dpi=dpi)
                poem_sort, loc_combine = acc_loc_poem(json_dict_1)
                acc_poem = get_poem(poem_sort, loc_combine)
                return acc_poem, img_save_path, json_dict_1
...
if __name__ == '__main__':
    ...
    for imgsdir in imgs_dir:
        ...
        inference_dir(project, imgsdir, word_save_dir, img_temp_dir, task_name=task_name, acc_loc=acc_loc, dpi=dpi, sort_way=sort_way)
